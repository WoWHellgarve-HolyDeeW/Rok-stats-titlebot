...
interface DashboardKingdom {
  ...
  recent_errors: { action: string; detail: string | null; created_at: string }[];
}
...
export default function AdminDashboardPage() {
  ...
  const [dashboardData, setDashboardData] = useState<DashboardKingdom[]>([]);
  ...
  const fetchDashboard = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setDashboardData(data.kingdoms || []);
      }
    } catch (err) {
    ...
    <div className="min-h-screen">
      ...
      <main className="container py-8">
        ...
        {dashboardData.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xl font-bold mb-4">Multi-Kingdom Overview</h2>
            <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
              {dashboardData.map((kd) => (
                <div key={kd.kingdom_number} className="card border-border hover:border-accent/40 transition-colors">
                  <div className="flex items-center justify-between mb-3">
                    <Link href={`/${kd.kingdom_number}/home`} className="font-bold text-lg hover:text-accent transition-colors">
                      KD {kd.kingdom_number}
                    </Link>
                    {kd.bot ? (
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                        kd.bot.status === "offline" ? "bg-gray-500/20 text-gray-400" :
                        kd.bot.status === "idle" ? "bg-yellow-500/20 text-yellow-400" :
                        "bg-green-500/20 text-green-400"
                      }`}>
                        {kd.bot.mode || kd.bot.status}
                      </span>
                    ) : (
                      <span className="px-2 py-0.5 rounded text-xs font-medium bg-gray-500/20 text-gray-400">offline</span>
                    )}
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center mb-3">
                    <div>
                      <p className="text-xs text-muted">Governors</p>
                      <p className="font-bold text-blue-400">{kd.governors}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted">Queue</p>
                      <p className="font-bold text-yellow-400">{kd.pending_titles}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted">Today</p>
                      <p className="font-bold text-green-400">{kd.completed_today}</p>
                    </div>
                  </div>
                  {kd.bot && kd.bot.progress != null && kd.bot.total != null && kd.bot.total > 0 && (
                    <div className="mb-2">
                      <div className="w-full bg-border rounded-full h-1.5">
                        <div className="bg-accent h-1.5 rounded-full transition-all" style={{ width: `${Math.min(100, (kd.bot.progress / kd.bot.total) * 100)}%` }} />
                      </div>
                      <p className="text-xs text-muted mt-1">{kd.bot.progress}/{kd.bot.total}</p>
                    </div>
                  )}
                  {kd.recent_errors.length > 0 && (
                    <div className="mt-2 space-y-1">
                      {kd.recent_errors.slice(0, 2).map((e, i) => (
                        <p key={i} className="text-xs text-red-400 truncate">⚠ {e.detail || e.action}</p>
                      ))}
                    </div>
                  )}
                  <p className="text-xs text-muted mt-2">
                    Last scan: {kd.last_scan ? new Date(kd.last_scan).toLocaleDateString() : "Never"}
                  </p>
                </div>
              ))}
            </div>
