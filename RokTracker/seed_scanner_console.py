...
def main():
    ...
    try:
        ...
        with single_instance_lock(lock_name, timeout_s=0.0) as acquired:
            ...
            seed_scanner = SeedScanner(bluestacks_port, config)
            seed_scanner.set_batch_callback(print_batch)

            console.print(
                f"The UUID of this scan is [green]{seed_scanner.run_id}[/green]",
                highlight=False,
            )
            signal.signal(signal.SIGINT, lambda _, __: ask_abort(seed_scanner))

            seed_scanner.start_scan(kingdom, scan_amount, save_formats)
