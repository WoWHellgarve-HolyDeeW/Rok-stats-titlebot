...
2. Detecta a tag da alliance no chat ANTES de abrir perfil
...
def load_api_config() -> Dict:
    """Load merged API config from shared + local override files."""
    try:
        return load_api_config_dict(get_app_root())
...
class Config:
    ...
    allowed_alliances: List[str] = field(default_factory=list)
...
UI = {
    ...
    "alliance_button_bar": (1160, 830),  # Alias usado no fluxo de give_title
    "members_tab": (105, 295),  # Ícone Members no menu lateral esquerdo do painel Alliance
    "search_icon": (700, 273),  # Campo de pesquisa visível directamente no ALLIANCE MEMBERS
    "search_field": (700, 273),
    # Fallback: quando o focus no input falha, clicar no centro do input
    "search_field_fallback": (400, 273),
...
class TitleBot:
    ...
    def process_single_request(self, req: dict) -> bool:
        ...
        try:
            ...
            alliance_tag = req['alliance_tag'] or ""
            title_type = req['title_type']
            
            prefix = f"[{alliance_tag}]" if alliance_tag else ""
            print(f"  Queue add: {prefix}{player_name} -> {title_type}", flush=True)
            ok, msg = self.api.create_title_request(player_name, alliance_tag, title_type)
        ...
        try:
            ...
            def gov_callback(gov, additional):
                ...
                try:
                    gov_data = {
                        ...
                        "Deads": gov.dead,
                        "T1 Kills": gov.t1_kills,
                        "T2 Kills": gov.t2_kills,
                        "T3 Kills": gov.t3_kills,
                        "T4 Kills": gov.t4_kills,
                        "T5 Kills": gov.t5_kills,
            ...
            if scanned_count > 0:
                try:
                    ...
                    print(f"  Failed to flush governor data: {flush_error}", flush=True)
