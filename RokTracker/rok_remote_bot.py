...
class RemoteBotClient:
    ...
    def __init__(self, config: BotConfig):
        ...
        self._scan_session_id: Optional[str] = None
        
        # Load bot API key from config or environment
        self._bot_key = os.getenv("BOT_API_KEY", "")
        try:
            api_config = load_api_config_dict(get_app_root())
    ...
    def upload_governor(self, gov_data: Dict[str, Any]):
        ...
        try:
            payload = dict(gov_data)
            if self._scan_session_id:
                payload["session_id"] = self._scan_session_id
            resp = requests.post(
    ...
    def begin_scan_session(self) -> str:
        self._scan_session_id = uuid.uuid4().hex
...
class RokRemoteBot:
    ...
    def run_scan(self, scan_type: str, options: Dict[str, Any]) -> bool:
        ...
        try:
            self.scanned_count = 0
            self.api_client.begin_scan_session()
            ...
            self.api_client.clear_scan_session()
    ...
    def run_title_bot(self, options: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            try:
                from roktracker.utils.config_loader import get_config
                tb_cfg = get_config().get("title_bot", {})
                mode = options.get("mode") or tb_cfg.get("mode", mode)
                sync_interval = options.get("sync_interval") or tb_cfg.get("sync_interval", sync_interval)
                cooldown = options.get("cooldown_seconds") or tb_cfg.get("cooldown_seconds", cooldown)
            except Exception:
                pass
                ...
                try:
                    ...
                    for player_name, alliance_tag, title_type in requests:
                        ...
                        if was_new:
                            ...
                            # In auto mode, grant the title via ADB navigation
                            if mode == "auto" and request_key not in processed_players:
                                processed_players.add(request_key)
                                console.print(f"[bold yellow]AUTO-GRANT: Giving {title_type} to {player_name}...[/bold yellow]")
                                self.set_state(BotState.GIVING_TITLES, f"Granting {title_type} to {player_name}")
                                
                                try:
                                    # Lazy-init TitleBot instance (reuse ADB connection)
                                    if not hasattr(self, '_title_bot'):
                                        from title_bot import TitleBot, Config as TitleConfig
                                        title_cfg = TitleConfig(
                                            adb_path=str(get_app_root() / "deps" / "platform-tools" / "adb.exe"),
                                            device_id=f"localhost:{self.config.bluestacks_port}",
                                            idle_reference=str(get_app_root() / "idle_reference.png"),
                                        )
                                        self._title_bot = TitleBot(title_cfg)
                                        self._title_bot.adb = self.adb  # share ADB connection
                                    
                                    success = self._title_bot.give_title(
                                        player_name=player_name,
                                        title_type=title_type,
                                    ...
                                    if success:
                                        console.print(f"[bold green]  ✓ {title_type} granted to {player_name}[/bold green]")
                                        # Report to API
                                        try:
                                            self.api_client.update_status(
                                                "giving_titles",
                                                f"Granted {title_type} to {player_name}"
                                            )
                                        except Exception:
                                            pass
                                    else:
                                        console.print(f"[red]  ✗ Failed to grant {title_type} to {player_name}[/red]")
                                        processed_players.discard(request_key)  # allow retry
                                    
                                    # Cooldown between grants
                                    time.sleep(cooldown)
                                    
                                except Exception as e:
                                    logger.error(f"Auto-grant failed for {player_name}: {e}")
                                    console.print(f"[red]  Auto-grant error: {e}[/red]")
                                    processed_players.discard(request_key)
