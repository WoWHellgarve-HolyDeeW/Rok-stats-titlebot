...
class KingdomScanner:
    def __init__(self, config, scan_options, port, adb_client=None):
        ...
        # Paths loaded from config.yaml
        try:
            from roktracker.utils.config_loader import get_path
            self.root_dir = get_path("root_dir") or get_app_root()
            self.tesseract_path = get_path("tesseract_data")
            self.img_path = get_path("temp_images")
            self.scan_path = get_path("scans.kingdom")
        except Exception:
        ...
        # Use provided ADB client or create new one
        if adb_client is not None:
            self.adb_client = adb_client
            self.external_adb = True  # Don't kill ADB if it was provided externally
        else:
    ...
    def scan_governor(
        ...
        while not (gov_info) and not self.stop_scan:
        ...
        # Check for stop before continuing to page 2
        if self.stop_scan:
            return governor_data
    ...
    def start_scan(
        ...
        # Only kill ADB if not in remote mode and not using external ADB client
        if not getattr(self, 'remote_mode', False) and not getattr(self, 'external_adb', False):
