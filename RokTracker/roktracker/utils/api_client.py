...
from roktracker.utils.local_api_config import (
    get_local_api_config_path,
    load_api_config_dict,
    write_api_config_dict,
)
...
def load_api_config(config_path: Optional[Path] = None) -> APIConfig:
    ...
    root_dir = config_path.parent if config_path else Path(__file__).resolve().parents[2]
    try:
        data = load_api_config_dict(root_dir)
        kingdom_number = data.get("kingdom_number") or data.get("primary_kingdom") or 0
        if not kingdom_number:
            kingdom_numbers = data.get("kingdom_numbers") or []
            if isinstance(kingdom_numbers, list) and kingdom_numbers:
                kingdom_number = kingdom_numbers[0]
        return APIConfig(
            base_url=data.get("api_url", "http://localhost:8000"),
            kingdom_number=int(kingdom_number or 0),
...
def save_api_config(config: APIConfig, config_path: Path) -> None:
    ...
    data = {
        ...
        "kingdom_numbers": [config.kingdom_number] if config.kingdom_number else [],
        "primary_kingdom": config.kingdom_number,
        "auto_upload": config.auto_upload,
        "batch_size": config.batch_size,
        "timeout": config.timeout,
    }
    target_path = get_local_api_config_path(config_path.parent) if config_path.name == "api_config.json" else config_path
    write_api_config_dict(target_path, data)
