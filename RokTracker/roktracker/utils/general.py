...
def load_cv2_img(path: str | PathLike[Any], flags: int) -> MatLike:
    ...
    # Check minimum expected dimensions (should be at least 1600x900)
    expected_min_width = 1500
    expected_min_height = 800
    
    height, width = img.shape[:2]
    if width < expected_min_width or height < expected_min_height:
        raise ValueError(f"Image too small: {width}x{height}, expected at least {expected_min_width}x{expected_min_height}")
