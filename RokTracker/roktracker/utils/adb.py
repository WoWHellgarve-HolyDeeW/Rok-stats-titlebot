...
from roktracker.utils.adb_lock import adb_interprocess_lock
...
class AdvancedAdbClient:
    ...
    def start_adb(self) -> None:
        ...
        device_key = f"localhost:{self.client_port}"
        with adb_interprocess_lock(device_key, timeout_s=30.0):
            process = subprocess.run(
                [
                    self.adb_path,
        ...
        for i in range(3):
            try:
                with adb_interprocess_lock(device_key, timeout_s=30.0):
                    ...
                    # Validate screenshot is not empty/too small
                    if result is None:
                        raise AdbError("takeSnapshot returned None")
                    width, height = result.size
                    if width < 100 or height < 100:
                        raise AdbError(f"Screenshot too small: {width}x{height}")
                    return result
            except Exception as e:
                last_error = e
                console.print(f"[red]ADB crashed (attempt {i+1}/3): {type(e).__name__}[/red]")
                self.kill_adb()
                self.start_adb()
        raise AdbError(f"Failed to capture screenshot after 3 attempts. Last error: {last_error}")
