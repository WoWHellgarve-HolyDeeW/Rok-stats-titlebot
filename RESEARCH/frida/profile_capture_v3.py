#!/usr/bin/env python3
"""
RoK Profile Stats Capture v3.0 — Dynamic ASLR-safe Lua VM burst-capture.

Uses .dynsym RVAs (constant across ASLR restarts) to dynamically resolve
Lua VM function addresses in libEngineDll.so.

When a profile-UI trigger string appears (txt_PowerNum, txt_KillNum, etc.),
enters BURST MODE for 5s capturing ALL integers, numbers, strings, and
field names. Then correlates setfield/getfield keys with nearby values.

Usage:
  python -u profile_capture_v3.py --pid 5500 --duration 300
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "captures", "profile")
os.makedirs(OUT_DIR, exist_ok=True)

# ─── Frida JS (ASLR-safe via .dynsym RVAs) ─────────────────────────────

JS_CODE = r"""
'use strict';

// Dynamic Lua VM address resolution from libEngineDll.so
// Uses .dynsym RVAs (constant across ASLR) + runtime base
var _m = Process.findModuleByName('libEngineDll.so');
if (!_m) throw new Error('libEngineDll.so not found!');
var _base = _m.base;
send({t:'info', msg:'libEngineDll.so base=' + _base + ' size=' + _m.size});
...
send({t:'info', msg:'Hooks: pushstr=' + LUA_PUSHSTRING + ' pushint=' + LUA_PUSHINTEGER + ' setfield=' + LUA_SETFIELD});
...
var TRIGGERS = [
    ...
    'help_times', 'troop_count', 'governor_id', 'nickname',
    'power', 'killpoints', 'dead', 'healed',
    'ranklist', 'rank_list', 'ranking_data',
    'individual_power_ranking', 'individual_kill_ranking',
    'kingdom_power', 'kingdom_kill',
    'TopPart', 'txt_Detail', 'TopBar', 'BottomPart',
];
...
Interceptor.attach(LUA_PUSHNUMBER, {
    onEnter: function(a) {
        // x86_64: double in register — read from context xmm0
        var v;
        try {
            // For x86_64 calling convention, floating-point args are in xmm registers
            v = a[1].toInt32(); // fallback: read as integer
        } catch(e) { v = 0; }
...
class ProfileCapture:
    ...
    def on_message(self, msg, data):
        ...
        if t == 'info':
            ...
            return
        if t == 'ready':
            print("  [READY] Profile capture active — hooks installed!", flush=True)
    ...
    def _analyze_burst(self, events, burst_id):
        ...
        for e in strs:
            ...
            elif re.search(r'power|kill|dead|troop|name|nick|guild|kingdom|vip|uid|governor|rank|heal|rss|gather|help|level|alliance', v, re.I):
                inter.append(v)
            elif len(v) >= 3 and not v.startswith('LC_') and not '/' in v:
                inter.append(v[:80])
        if inter:
            print(f"  Interesting: {inter[:30]}", flush=True)
            ...
            if e['t'] in ('setf','getf') and isinstance(e.get('v'), str):
                ...
                for j in range(i+1, min(i+4, len(events))):
                    ...
                    if ne['t'] in ('str','tol') and isinstance(ne.get('v'),str) and re.match(r'^[\d,]+$', ne['v']):
                        profile[key] = ne['v']; break
                    if ne['t'] in ('str','tol','lstr') and isinstance(ne.get('v'),str) and len(ne['v']) >= 2 and not ne['v'].startswith(('UI/','Content/','LC_')):
                        profile[key] = ne['v'][:200]; break
    ...
    def save_final(self):
        ...
        print(f"  ProStrs  : {len(self.profile_strs)}", flush=True)
    ...
    def run(self, pid, duration):
        print(f"""
{'='*60}
  RoK Profile Stats Capture v3.0 (ASLR-safe Lua VM burst)
  PID: {pid} | Duration: {duration}s
  Output: {OUT_DIR}
{'='*60}

  Open player profiles in-game to trigger capture.
  Look for: power rankings, player profiles, kill stats
""", flush=True)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='RoK Profile Stats Capture v3.0')
    parser.add_argument('--pid', type=int, default=12401)
